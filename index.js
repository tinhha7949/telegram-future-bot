    ...
    `✅ BINANCE TPSL CLEAR CONFIRMED ${symbol}`
        ...
        try{
            ...
            try{
                ...
                await checkTimeError(cancelError)

                console.log(
                    `⚠️ FAILED CANCEL SL ${symbol}:`,
                    cancelError.message
                )
            }
        ...
        const hasSL =
            finalOrders.some(
                o =>
                    o.orderId === slRes?.orderId
            )

        const hasTP =
            finalOrders.some(
                o =>
                    o.orderId === tpRes?.orderId
            )
...
if(!saved){
    ...
    console.log(
        `🚨 TRADE STATE SAVE FAIL ${trade.symbol}`
    )
...
async function updateTradeTPSLData(trade){
    ...
    try{
            ...
            await trades.updateOne(
...

                {
                    $set: {
                        ...
                        enteredAt:
                            Number(trade.enteredAt || Date.now()),
        ...
        if(result.matchedCount === 0){
            ...
            console.log(
                `❌ TRADE NOT FOUND DB ${trade.symbol}`
            )
        ...
        console.log(
            `💾 TRADE STATE SAVED ${trade.symbol}`
        )
...
async function runDynamicTPSL(){
    ...
    if(DYNAMIC_TPSL_RUNNING){
        console.log("⛔ DYNAMIC TPSL ALREADY RUNNING")
        return
    }
